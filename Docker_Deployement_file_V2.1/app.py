...
def detect_and_crop_head_shoulders(image_path):
    ...
    if len(result) == 1:
        ...
        # Adjust the cropping area to include the head and shoulders
        # You can modify these values based on your specific needs
        y -= int(h * 0.2)  # Move the top edge up
        h = int(h * 1.4)   # Increase the height to include the shoulders
...
def predict_age_from_image(image_path, model):
    cropped_img = detect_and_crop_head_shoulders(image_path)
